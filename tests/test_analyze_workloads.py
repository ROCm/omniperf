...
def test_analyze_vcopy_MI100():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv", ["omniperf", "analyze", "--path", "tests/workloads/vcopy/MI100"]
        ):
            omniperf.main()
...
def test_analyze_vcopy_MI200():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv", ["omniperf", "analyze", "--path", "tests/workloads/vcopy/MI200"]
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_ipblocks_TCP_MI300X_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/ipblocks_TCP/MI300X_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_ipblocks_TCP_MI300A_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/ipblocks_TCP/MI300A_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_ipblocks_SQC_MI300X_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/ipblocks_SQC/MI300X_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_ipblocks_SQC_MI300A_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/ipblocks_SQC/MI300A_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_ipblocks_TCC_MI300X_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/ipblocks_TCC/MI300X_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_ipblocks_TCC_MI300A_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/ipblocks_TCC/MI300A_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_dispatch_0_MI300X_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/dispatch_0/MI300X_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_dispatch_0_MI300A_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/dispatch_0/MI300A_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_kernel_MI300X_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/kernel/MI300X_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_kernel_MI300A_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/kernel/MI300A_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_kernel_substr_MI300X_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/kernel_substr/MI300X_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_kernel_substr_MI300A_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/kernel_substr/MI300A_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_dispatch_7_MI300X_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/dispatch_7/MI300X_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_ipblocks_TA_MI300X_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/ipblocks_TA/MI300X_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_ipblocks_TA_MI300A_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/ipblocks_TA/MI300A_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_ipblocks_TD_MI300X_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/ipblocks_TD/MI300X_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_ipblocks_TD_MI300A_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/ipblocks_TD/MI300A_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_device_filter_MI300X_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/device_filter/MI300X_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_device_filter_MI300A_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/device_filter/MI300A_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_ipblocks_CPF_MI300X_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/ipblocks_CPF/MI300X_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_ipblocks_CPF_MI300A_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/ipblocks_CPF/MI300A_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_dispatch_inv_MI300X_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/dispatch_inv/MI300X_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_dispatch_inv_MI300A_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/dispatch_inv/MI300A_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_path_MI300X_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/path/MI300X_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
...
def test_analyze_ipblocks_SPI_MI300X_A1():
    with pytest.raises(SystemExit) as e:
        with patch(
            "sys.argv",
            ["omniperf", "analyze", "--path", "tests/workloads/ipblocks_SPI/MI300X_A1"],
        ):
            omniperf.main()
    assert e.value.code == 0
